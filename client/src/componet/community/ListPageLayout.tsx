...
const ListPageLayout = ({id}) => {
	...
	const getReady = async () => {
		let res = await getCommunity(id?.id);
		if(res) {
			setList(res)
			setLoading(false);
		}
	...
	const onHandleClick = async () => {
		navigate(`/community/write`, { state: { id: id?.id,actType: 'create' } });
	}
											...
											<tr className={`align-middle`} key={item.id} className="community-table-row">
												...
												<td className="py-3 text-center">
													<span
														...
														onClick={() => navigate('/community/detail', { state: { id: item.id } })}
													>{item.title}</span>
												</td>
